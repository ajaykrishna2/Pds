...
class read_input_for_reconcilation:
    if((datetime.datetime.now().month)!=1):
            ...
            month_name = calendar.month_name[month]
    @classmethod
    def collect_and_preprocess_riverview_data(cls):
        try:
            ...
            data = cli.get_object(Bucket=bucket_name, Key=reconcilation_input+"/Rec_Riverview/"+str(cls.year)+"/"+cls.month_name+"/"+"RVH_Inven_RECON_062921_34626.TXT")
            data = data['Body'].read()
            ...
            logging.info("read Riverview_reconcilation_statement file successfully")
