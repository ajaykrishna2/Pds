...
class read_input_for_reconcilation:
    ...
    def collect_and_preprocess_womenshospital_data(self):
        try:
            ...
            print(cli)
            data = cli.get_object(Bucket=bucket_name, Key=reconcilation_input+"/Rec_Womens_Hospital"+"/"+str(read_input_for_reconcilation.year)+"/"+read_input_for_reconcilation.month_name+"/"+"TWH_RECON_060221_53023.TXT")
            data = data['Body'].read()
            ...
            logging.info("read Womens_reconcilation_statement file successfully")
    ...
    def info_out_read(self):
        try:
            session = boto3.Session(aws_access_key_id=aws_access_key, aws_secret_access_key=aws_secret_key,region_name=bucket_region);
            cli=session.client('s3')
            data1 = cli.get_object(Bucket=bucket_name, Key=reconcilation_input+"/Rec_Womens_Hospital"+"/"+str(read_input_for_reconcilation.year)+"/"+read_input_for_reconcilation.month_name+"/"+"info-out_PUT_in_FACS.TXT")
