...
class harrison_reconcilation_report:
    def Harrison(self,df,inf):
        # try:
            ...
            # date_srm = pd.to_datetime(pd.Series(inf['send dt/tm']))
            # inf['send dt/tm'] = date_srm.dt.strftime('%m-%d-%Y')
            ...
            # print(df[df['Issue Detected'] == "Account is in Harrison CO File, but it is not in FACS."])
            ...
            df14['Send Dt/Tm'] = df14['Send Dt/Tm'].astype(float)
            df14['Med-1 Balance'] = df14['Send Dt/Tm']
            print(df14.to_string())
            df14['Match?'] = np.where(df14['Med-1 Balance'] == df14['Recon file (Harrison) balance'], 'True', 'False')
            df14.loc[df14['Match?'] == 'True', 'Issue Detected'] = 'Balance Match'
            df14.drop('Match?', axis=1, inplace=True)
            dfm14 = df14[df14['Issue Detected'] == "Balance Match"]
            df14.drop(dfm14.index,inplace=True)
            df15=pd.concat([df14, df11])
            df16=pd.concat([df12, df13,dfm14])
            # print(inf[inf['Cancel Code']==''])
            ...
            inf2= inf.rename(columns={'Issue Deteched':'Issue Detected','FACS Acct': 'FACS #', 'Client REF #': 'CLIENT ACCOUNT #','List DTE': 'List Date','AMT Cancelled':'Amount Cancelled','Med1 BAL':'Med-1 Balance','Recon File BAL':'Recon file (Harrison) balance','Cancel DTE':'Cancel Date','Last Pay DTE':'Last Payment Date','send dt/tm':'Send Dt/Tm'}, inplace=False)
            ...
            inf3=inf3[['Issue Detected_x','CLIENT ACCOUNT #','Recon file (Harrison) balance_x','Harrison F/C','Send Dt/Tm_x','FACS #_y','Client ID_y','First Name_y','Last Name_y','Disposition_y','Phase_y','List Date_y','Med-1 Balance_y','Amount Cancelled_y','Recon file (Harrison) balance_y','Cancel Code_y','Cancel Description_y','Cancel Date_y','Last Payment Date_y']]
            inf3 = inf3.rename(columns={'Issue Detected_x':'Issue Detected', 'Recon file (Harrison) balance_x':'Recon file (Harrison) balance', 'Send Dt/Tm_x':'Send Dt/Tm', 'FACS #_y':'FACS #', 'Client ID_y':'Client ID',
                 ...
                 'Amount Cancelled_y':'Amount Cancelled',  'Cancel Code_y':'Cancel Code','Cancel Description_y': 'Cancel Description',
            ...
            print(inf3.sort_values('CLIENT ACCOUNT #').to_string())
            print(len(inf3))
            ...
            df7.drop(df7[~(df7['Phase'] == 20) ].index,inplace=True)
            ...
            df25 = df20[ ((df20['Cancel Code'].isna()))]
            # dfm25=df20[(df20['Recon file (Harrison) balance']<0)&((df20['Cancel Code']==2))]
            dfm25 = df20[((df20['Cancel Code'] == 2))]
            df25["Issue Detected"].replace({"Acct is in Harrison file but closed in FACS": "Balance Mismatch"},inplace=True)
            dfm25["Issue Detected"].replace({"Acct is in Harrison file but closed in FACS": "Balance Mismatch"},
                                           inplace=True)
            df26=pd.concat([df24,df25,dfm25])
            df26['Phase']=pd.to_numeric(df26['Phase'], errors='coerce').astype(float)

            df26.drop(df26[(df26['Phase'].isna())].index, inplace=True)

            df20.drop(df25.index,inplace=True)
            df20.drop(dfm25.index, inplace=True)
            df20.drop(df20[(df20['Cancel Code']==4)|(df20['Cancel Code']==71)].index,inplace=True)
            df23.reset_index(inplace=True)
            df23['Phase'] = pd.to_numeric(df23['Phase'], errors='coerce').astype(float)
            df23.drop(df23[(df23['Phase'].isna())].index, inplace=True)
            ...
            save_file.send_statement(df20, 'Closed in FACS','HA4_MVA_HOSP','Rec_Harrison_Hospital')
            save_file.send_statement(df7, 'FACS not RECON','HA4_MVA_HOSP','Rec_Harrison_Hospital')
            save_file.send_statement(df_r, 'RECON not FACS','HA4_MVA_HOSP','Rec_Harrison_Hospital')
            save_file.send_statement(df26, 'Balance Mismatch','HA4_MVA_HOSP','Rec_Harrison_Hospital')
            save_file.send_statement(df23, 'Balance Match','HA4_MVA_HOSP','Rec_Harrison_Hospital')
            save_file.send_statement(dfw1, 'Multiples','HA4_MVA_HOSP','Rec_Harrison_Hospital')
