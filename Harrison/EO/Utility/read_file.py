...
class read_input_for_reconcilation:
    ...
    def collect_and_preprocess_harrison_data(self):
        try:
            ...
            data = cli.get_object(Bucket=bucket_name, Key=reconcilation_input+"/Rec_Harrison_Hospital/"+str(read_input_for_reconcilation.year)+"/"+read_input_for_reconcilation.month_name+"/"+"HA2_EO_HOSP_PHY_RECON_090221_32868.TXT")
            data = data['Body'].read()
    ...
    def info_out_read(self):
        try:
            session = boto3.Session(aws_access_key_id=aws_access_key, aws_secret_access_key=aws_secret_key,region_name=bucket_region);
            cli=session.client('s3')
            data1 = cli.get_object(Bucket=bucket_name, Key=reconcilation_input+"/Rec_Harrison_Hospital/"+str(read_input_for_reconcilation.year)+"/"+read_input_for_reconcilation.month_name+"/"+"HCH_EO_info-out_PUT_in_FACS.TXT")
