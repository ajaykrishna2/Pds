...
class read_input_for_reconcilation:
    if((datetime.datetime.now().month)!=1):
            ...
            month_name = calendar.month_name[month]
    @classmethod
    def collect_and_preprocess_kingsdaughter_data(cls):
        try:
            session = boto3.Session(aws_access_key_id=aws_access_key, aws_secret_access_key=aws_secret_key,
                                    region_name=bucket_region);
            cli = session.client('s3')
            data = cli.get_object(Bucket=bucket_name, Key=reconcilation_input + "/Rec_Kings_Daughter/" + str(cls.year) + "/" + cls.month_name + "/" + "KDH_CBSO_REC_071621_7700.TXT")
            data = data['Body'].read()
            ...
            logging.info("read Kingsdaughter_reconcilation_statement file successfully")
