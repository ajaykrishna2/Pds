...
class read:
    ...
    def collect_and_preprocess_harrison_bd_data(self):
        try:
            ...
            for i in filtered_list:
              if 'Rec_Harrison_Hospital' in i and str(read.year) in i and read.month_name in i and 'HA1_BD_Incoming' in i:
                ...
                data = obj['Body'].read()
                harrison_bd_reconcilation_statement = pd.read_table(io.BytesIO(data),sep="\t",index_col=False, encoding="ISO-8859-1")
            return harrison_bd_reconcilation_statement

   
        except Exception as e:
            logging.exception("error in reading harrison_bd_reconcilation file")
    def info_out_read(self):
        try:
            ...
            filtered_list =list( filter(lambda x:'Reconcilation_Input' in x,list_of_files))
            for i in filtered_list:
              if 'Rec_Harrison_Hospital' in i and str(read.year) in i and read.month_name in i and 'Info-out_BD' in i:
                client = boto3.client('s3',aws_access_key_id= aws_access_key, aws_secret_access_key=aws_secret_key,
                region_name=bucket_region)
                obj = client.get_object(Bucket=bucket_name, Key=i)
                data = obj['Body'].read()
            ...
            logging.exception("error in reading harrison_info_reconcilation file")
