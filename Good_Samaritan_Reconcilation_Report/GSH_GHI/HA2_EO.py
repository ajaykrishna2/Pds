from Utility.harrison_eo_reconcilation import *
...
class Harrison_EO_reconcilation:
    print("starting data ::::::::::: ",datetime.datetime.now())
    # creation of log file
    logging.basicConfig(filename=local_output_directory+'Harrison_EO_reconcilation.log',filemode='a', level=logging.DEBUG,
                       format='%(asctime)s %(levelname)s %(name)s %(message)s')
    #Read required input
    obj = read()
    reconcilation_read = obj.collect_and_preprocess_harrison_eo_data()
    info_out_read=obj.info_out_read()
    reconcilation = harrison_eo_reconcilation_report()
    reconcilation.Harrison_EO(reconcilation_read,info_out_read)
